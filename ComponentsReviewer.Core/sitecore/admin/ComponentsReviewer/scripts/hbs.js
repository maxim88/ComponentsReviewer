...
define([
    ...
    'helper', 'handlebars', 'underscore', 'json2'
    //>>excludeEnd('excludeHbs')
], function (
    //>>excludeStart('excludeHbs', pragmas.excludeHbs)
    helper, Handlebars, _, JSON
    //>>excludeEnd('excludeHbs')
) {
        //>>excludeStart('excludeHbs', pragmas.excludeHbs)
        ...
        Handlebars.registerHelper('update_html', function (text, opts) {
            return helper.stringToHtml(text);
        });
        ...
        Handlebars.registerHelper('replace_csDateTime', function (val, opts) {

            return helper.getDate(val);
        });

        Handlebars.registerHelper('get_DateTime', function (val, opts) {
            return helper.toDurationTime(val);
        });

        Handlebars.registerHelper('is_checked', function (val, opts) {
            return val ? "checked" : "";
        });

        Handlebars.registerHelper('replace_value', function (text, value, warning, critical, expectLow, valueType, isCritical, name, machine, date, opts) {
            var dateJs = new Date(parseInt(date.substr(6)));
            var d = helper.dateToFormat(dateJs);
            return helper.replaceProblemValue(text, value, warning, critical, expectLow, valueType, isCritical, name, machine, d);
        });

        Handlebars.registerHelper('replace_tooltips', function (description, machine, date, opts) {
            var dateJs = new Date(parseInt(date.substr(6)));
            var d = helper.dateToFormat(dateJs);
            return helper.replaceProblemDescription(description, machine, d);
        });


        Handlebars.registerHelper('highlight_log', function (text, opts) {
            return text.replace("ERROR", "<b class='red'>ERROR</b>").replace("WARN", "<b class='yellow'>WARN</b>");
        });
        ...
        Handlebars.registerHelper('if_more', function (a, b, opts) {
            if (a > b) // Or === depending on your needs
                return opts.fn(this);
            else
                return opts.inverse(this);
        ...
        Handlebars.registerHelper('toMemory', function (a, opts) {
            var e = "2.000.396.742656";
